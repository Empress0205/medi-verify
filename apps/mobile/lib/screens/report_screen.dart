...
class _ReportScreenState extends State<ReportScreen> {
  ...
  String? _reportCode; // real code returned by the backend
  ...
  Widget build(BuildContext context) {
    if (_submitted) return _SuccessScreen(reportCode: _reportCode);
    ...
    try {
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          ...
          margin: const EdgeInsets.all(16),
          content: Text(
            'Could not submit report: ${e.toString().replaceFirst('Exception: ', '')}',
...
class _SuccessScreen extends StatelessWidget {
  ...
  const _SuccessScreen({this.reportCode});
